...
def get_cart_sellers(bucket_id):
    ...
    for seller_id, seller_data in sellers.items():
        ...
        user_info = conn.execute(
            'SELECT username, first_name, last_name, created_at FROM users WHERE id = ?',
            (seller_id,)
        ).fetchone()
        if user_info:
            display_name = f"{user_info['first_name'] or ''} {user_info['last_name'] or ''}".strip()
            seller_data['display_name'] = display_name or user_info['username']
            ...
            seller_data['display_name'] = seller_data['username']
            seller_data['member_since'] = None
        ...
        ''', (seller_id,)).fetchone()
        seller_data['transaction_count'] = tx_result['transaction_count'] if tx_result else 0
        ...
        seller_data['repeat_buyers_pct'] = round((repeat_buyers / total_buyers) * 100) if total_buyers else None
        ...
        ''', (seller_id,)).fetchone()
        avg_days = ship_result['avg_days'] if ship_result and ship_result['avg_days'] is not None else None
