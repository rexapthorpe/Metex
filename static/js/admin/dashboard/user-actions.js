...
function confirmFreezeAction() {
  if (!pendingAction || pendingAction.type !== 'freeze') return;
  ...
  const endpoint = `/admin/api/user/${pendingAction.userId}/freeze`;
  ...
  fetch(endpoint, {
    ...
    body: JSON.stringify({ action: 'freeze', reason: reason })
  })
...
function confirmAction() {
  ...
  fetch(endpoint, fetchOptions)
    ...
    .then(data => {
      ...
      if (data.success) {
        // For delete action, show success animation
        if (actionType === 'delete') {
          closeUserModal();
          showConfirmSuccess(
            'User Deleted',
